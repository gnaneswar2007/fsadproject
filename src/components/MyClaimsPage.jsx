...
export function MyClaimsPage() {
    ...
    const load = async () => {
        ...
        const claimedIdSet = new Set(claimedIds.map((id) => String(id)));

        // Snapshot cached donations before getDonations refreshes cache.
        let cachedDonations = [];
        try {
            cachedDonations = JSON.parse(localStorage.getItem("api_donations_cache") || "[]");
        } catch { cachedDonations = []; }

        // Fetch latest donations and also include cached records for recipient views.
        const allDonations = await getDonations();

        const mergedById = new Map();
        [...cachedDonations, ...allDonations].forEach((donation) => {
            if (!donation?.id) return;
            mergedById.set(String(donation.id), donation);
        });

        const myClaimedDonations = Array.from(mergedById.values()).filter((d) => claimedIdSet.has(String(d.id)));
