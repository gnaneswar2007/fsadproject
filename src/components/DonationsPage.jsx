...
export function DonationsPage() {
  ...
  const showClaimedOnly = new URLSearchParams(location.search).get("view") === "claims";
  ...
  const fetchDonations = async () => {
    ...
    if (role === "admin" || role === "analyst") {
      ...
      setDonations(showClaimedOnly ? all.filter((d) => ["claimed", "picked_up"].includes(d.status)) : all);
    } else {
      const mine = await getDonationsByUser(user.id);
      setDonations(showClaimedOnly ? mine.filter((d) => ["claimed", "picked_up"].includes(d.status)) : mine);
    }
    setLoading(false);
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">
            {showClaimedOnly ? "Claims Made" : role === "admin" || role === "analyst" ? "All Donations" : "My Donations"}
          </h1>
          <p className="text-sm text-muted-foreground">
            {showClaimedOnly
              ? "Donations that have been claimed or marked as picked up"
              : role === "admin" || role === "analyst"
                ? "View all donations on the platform"
                : "List and manage your food donations"}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {(role === "admin" || role === "analyst") && <ExportMenu data={donations} filename={showClaimedOnly ? "claimed-donations" : "donations"} pdfTitle={showClaimedOnly ? "Claims Report" : "Donations Report"} />}
          {!showClaimedOnly && role !== "admin" && role !== "analyst" && <ListDonationDialog onSuccess={fetchDonations} triggerLabel="New Donation" />}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border bg-card py-16 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {showClaimedOnly ? "No claimed donations yet." : "Click \"New Donation\" to list your first surplus food item."}
          </p>
        </div>
      ) : (
